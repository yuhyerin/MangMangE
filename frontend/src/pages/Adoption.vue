  ...
  <div>
    ...
    <div class="container" style="padding-top: 80px; width: 70%">
      ...
      <div style="border-bottom: 2.5px solid #bbb; padding-bottom: 2%;"> <!-- 신청 폼 -->
        <v-row md="12" style="padding: 12px">
          ...
          <v-col style="margin-left: 3%;">
            ...
            <v-card outlined>
              <v-card-text>
                1. 입양신청서의 이름과 회원정보의 이름이 불일치할 경우 입양에 불이익이 있을 수 있습니다.<br><br>
      ...
      <div> <!-- 개인정보 수집 내용 -->
        ...
        <p>(주)댕줍멍줍은 아래의 목적으로 개인정보를 수집 및 이용하며, 회원의 개인정보를 안전하게 취급하는데 최선을 다합니다.</p>
        <br>
        <v-card outlined>
        ...
        <v-checkbox v-model="agree" label="개인정보 이용에 동의합니다." color="#017648"></v-checkbox>
      </div> <!-- 개인정보 끝 -->
      <div style="display: flex;"> <!-- 버튼 -->
        <div style="display:flex; justify-content: center; width:100%;">
          <v-btn dark rounded color="#017648" width="80%" @click="checkForm()">신청하기</v-btn>
        </div>
      <div style="display:flex; justify-content: center; width:100%;">
      <v-btn dark rounded color="black" width="80%" @click="$router.go(-1)">돌아가기</v-btn>
      </div>
      </div>
    </div>
  </div>
...
export default {
  ...
  methods: {
    ...
    checkForm(){
      ...
      err &&
      !this.phoneNumber &&
      ((msg="연락처를 입력해주세요."),(err=false),this.$refs.phoneNumber.focus());
      err &&
      !this.send &&
      ((msg="인증절차를 완료해주세요."),(err=false),this.$refs.phoneNumber.focus());
      err &&
      !this.auth &&
      ((msg="인증절차를 완료해주세요."),(err=false),this.$refs.message.focus());
      err &&
      !this.agree &&
